...
def extract_config_from_params(params: Dict, input_dim: int, cat_config: CategoricalConfig = None) -> TrainConfig:
...
def run_multi_seed_evaluation(
	...
	with mlflow.start_run(run_name="phase3_multi_seed"):
		...
		for i, trial in enumerate(top_trials):
			config = extract_config_from_params(trial.params, input_dim, cat_config)
...
def compare_and_save_model(
	...
	if existing_model is None:
		...
		try:
			...
			if old_feature_cols and set(old_feature_cols) != set(feature_cols):
				...
				data_test_old = prepare_data(df, feature_cols, [test_season], scaler=scaler_for_existing)
	...
	if save_new:
		...
		# Serialize cat_config if present
...
def train_final_model(
	...
	Approach to prevent overfitting:
	1. Use final_val_season (last season before test) for early stopping
	2. Train on remaining seasons, validate on final_val_season to find best_epoch
	...
	# Use last CV season as final validation for early stopping
	...
	with mlflow.start_run(run_name="phase4_final_model"):
		...
		# === Step 2: Retrain on all CV seasons for exactly best_epoch epochs ===
		...
		config.patience = best_epoch + 1  # Effectively disable early stopping
...
def main():
	...
	# === Phase 2: Refinement search ===
	refine_study = run_refinement_search(fold_data, input_dim, coarse_study, cat_config)
	
	# === Phase 3: Multi-seed evaluation ===
	best_config, best_result, _ = run_multi_seed_evaluation(
		refine_study, fold_data, input_dim, cat_config
	)
	
	# === Phase 4: Final model ===
	# Note: best_epoch is now determined inside train_final_model via early stopping
	# on final_val_season to prevent overfitting
	model, scaler, test_metrics = train_final_model(
		best_config, df, feature_cols, folds, test_season, cat_config
	)
