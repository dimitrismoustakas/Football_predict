...
These features are computed from **player-level match statistics** sourced from Understat. They aggregate individual player performance into team-level rolling features that capture team ability, concentration of talent, and squad depth.
...
- **HHI = 1**: One player accounts for 100% of the output
- **HHI = 0.1**: Output evenly distributed among 10 players
- **HHI = 0.5**: Dominated by 2 players

| Feature Name | Formula | Description |
|--------------|---------|-------------|
| `xg_hhi_r15` | `mean(sum(player_xg²) / sum(player_xg)²)` | Concentration of xG production. High = star-dependent |
| `xa_hhi_r15` | `mean(sum(player_xa²) / sum(player_xa)²)` | Concentration of xA production. High = playmaker-dependent |
| `minutes_hhi_r15` | `mean(sum(player_min²) / sum(player_min)²)` | Concentration of minutes. High = less rotation |
...
```python
xg_hhi = sum_xg_squared / team_total_xg² (if team_total_xg > 0, else null)
xa_hhi = sum_xa_squared / team_total_xa² (if team_total_xa > 0, else null)
minutes_hhi = sum_minutes_squared / team_total_minutes²
```
...
|--------|------|-----------|-------|
| `home_team_xg_r15` | Float64 | ~2-3% | First 1 game per team is null |
| `home_team_xa_r15` | Float64 | ~2-3% | |
| `home_team_total_goals_r15` | Float64 | ~2-3% | |
| `home_team_total_assists_r15` | Float64 | ~2-3% | |
| `home_team_total_shots_r15` | Float64 | ~2-3% | |
| `home_team_total_key_passes_r15` | Float64 | ~2-3% | |
| `home_team_total_minutes_r15` | Float64 | ~2-3% | |
| `home_xg_hhi_r15` | Float64 | ~2-3% | Can be null if team had 0 xG |
| `home_xa_hhi_r15` | Float64 | ~2-3% | Can be null if team had 0 xA |
| `home_minutes_hhi_r15` | Float64 | ~2-3% | |
| `home_unique_players_r15` | Float64 | ~2-3% | |
| `home_unique_players_r5_sum` | Float64 | ~2-3% | 5-game rolling sum |
...
1. **GER-Bundesliga 2024-25**: Missing due to Understat API format issue
2. **First games per team**: Null features (waiting for min_samples)
3. **Cross-season**: Rolling window resets at season boundaries (by design)